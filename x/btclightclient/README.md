...
ybtc chain needs to know about different events happening on Bitcoin chain.
To make it possible in a secure way, ybtc needs to know the current
...
- [Concepts](#concepts)
  ...
  - [ybtc's BTC light client design](#ybtcs-btc-light-client-design)
- [States](#states)
...
The ybtc chain needs to learn and validate a number of events that had
happened on the Bitcoin chain. Those events are:

1. `New checkpoint event` - Bitcoin Timestamping protocol requires checkpoints
on Bitcoin to be reported back to ybtc. To do it securely, each checkpoint
...
To properly validate those inclusion proofs, the ybtc chain needs to know the
current state of BTC chain i.e., what is current canonical chain recognized by BTC.

### ybtc's BTC light client design

ybtc maintains a BTC light client so that it can verify the inclusion
of various Bitcoin transactions.

In a high-level overview, the ybtc BTC light client starts from some base BTC
...
The base BTC header is defined in the [genesis](../../proto/ybtc/btclightclient/v1/genesis.proto) module.

The ybtc BTC light client module stores only BTC headers from the canonical
...
[object](../../proto/ybtc/btclightclient/v1/params.proto) defined as follows:
...
[object](../../proto/ybtc/btclightclient/v1/btclightclient.proto)
...
message BTCHeaderInfo {
  bytes header = 1
      [ (gogoproto.customtype) =
            "github.com/amovidhussaini/ybtcclone/types.BTCHeaderBytes" ];
  bytes hash = 2
      [ (gogoproto.customtype) =
            "github.com/amovidhussaini/ybtcclone/types.BTCHeaderHashBytes" ];
  uint32 height = 3;
  bytes work = 4
...
Its purpose is to update the state of the BTC chain as viewed by ybtc chain.
...
Upon receiving a `MsgInsertHeaders` message, a ybtc node applies the following
