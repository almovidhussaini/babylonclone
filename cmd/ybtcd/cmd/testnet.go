...
func TestnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a ybtc testnet",
...
Example:
	ybtcd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "ybtcd", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	ybtcConfig := DefaultybtcAppConfig()
	ybtcConfig.MinGasPrices = minGasPrices
	ybtcConfig.API.Enable = true
	ybtcConfig.API.Address = "tcp://0.0.0.0:1317"
	ybtcConfig.Telemetry.Enabled = true
	ybtcConfig.Telemetry.PrometheusRetentionTime = 60
	ybtcConfig.Telemetry.EnableHostnameLabel = false
	ybtcConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	// BTC related config. Default values "simnet" and "BBT1"
	ybtcConfig.BtcConfig.Network = btcNetwork
	// Explorer related config. Allow CORS connections.
	ybtcConfig.API.EnableUnsafeCORS = true
	ybtcConfig.GRPC.Enable = true
	ybtcConfig.GRPC.Address = "0.0.0.0:9090"

	// Disable IAVL cache by default as ybtc leaf nodes can be large, and in case
	// of big cache values, ybtc node can run out of memory.
	ybtcConfig.IAVLCacheSize = 0
	ybtcConfig.IAVLDisableFastNode = true
	...
	for i := 0; i < numValidators; i++ {
		...
		customTemplate := DefaultybtcTemplate()
		srvconfig.SetConfigTemplate(customTemplate)
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), ybtcConfig)
