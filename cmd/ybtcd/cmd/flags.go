...
import (
	...
	btcltypes "github.com/amovidhussaini/ybtcclone/x/btclightclient/types"
	btcstypes "github.com/amovidhussaini/ybtcclone/x/btcstaking/types"
...
func addGenesisFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String(flagCheckpointTag, btcctypes.DefaultCheckpointTag, "Hex encoded tag for ybtc checkpoint on btc")
	// epoch args
	...
	cmd.Flags().String(flagAllowedReporterAddresses, strings.Join(btcltypes.DefaultParams().InsertHeadersAllowList, ","), "addresses of reporters allowed to submit Bitcoin headers to ybtc")
	...
	// blocks args
	cmd.Flags().Int64(flagBlockGasLimit, ybtcApp.DefaultGasLimit, "Block gas limit")
	cmd.Flags().Int64(flagVoteExtensionEnableHeight, ybtcApp.DefaultVoteExtensionsEnableHeight, "Vote extension enable height")
