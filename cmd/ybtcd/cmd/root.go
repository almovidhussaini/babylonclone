...
import (
	...
	"github.com/amovidhussaini/ybtcclone/cmd/ybtcd/cmd/genhelpers"
)

// NewRootCmd creates a new root command for ybtcd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	// we "pre"-instantiate the application for getting the injected/configured encoding configuration
	// note, this is not necessary when using app wiring, as depinject can be directly used
	tempApp := app.NewTmpybtcApp()
...
func initAppConfig() (string, interface{}) {
	...
	ybtcTemplate := DefaultybtcTemplate()
	return ybtcTemplate, ybtcConfig
}
...
func appExport(
	...
	if height != -1 {
		ybtcApp = app.NewybtcApp(logger, db, traceStore, false, map[int64]bool{}, uint(1), privSigner, appOpts, app.EmptyWasmOpts)

		if err = ybtcApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		ybtcApp = app.NewybtcApp(logger, db, traceStore, true, map[int64]bool{}, uint(1), privSigner, appOpts, app.EmptyWasmOpts)
	}

	return ybtcApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}
